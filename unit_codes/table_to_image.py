...
def create_summary_table(final_data):
    ...
    df['detected_objects'] = df['detected_objects'].apply(lambda x: ", ".join([f"{obj['label']} ({obj['confidence']:.2f})" for obj in x]))
...
def plot_annotated_image(img, final_data):
    ...
    ax.imshow(img)
    ax.set_title("Original Image with Annotations")

    for data in final_data:
        object_id = data['object_id']
        bbox = [50, 50, 100, 100]  # Placeholder bounding box
        rect = Rectangle((bbox[0], bbox[1]), bbox[2], bbox[3], linewidth=2, edgecolor='r', facecolor='none')
        ax.add_patch(rect)
        ax.text(bbox[0], bbox[1] - 10, f"ID: {object_id}", color='white', fontsize=12, bbox=dict(facecolor='red', alpha=0.5))
...
if __name__ == '__main__':
    master_id = '8fb16678-83cf-4daf-81bb-179bf7ef10a3'
    image_path = 'uploaded_images/images.jpg'
